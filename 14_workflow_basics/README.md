...
Calls two Foundry-backed agents from a plain `@workflow`. Each model call is an `@step`, so its result can be reused during resume/checkpoint scenarios instead of paying for the call again.
