...
1. `demo.py` defines a typed function with `@tool`, adds it to an agent, and lets the Foundry model call it. `never_require` is used only because this weather tool is read-only and has no side effects.
