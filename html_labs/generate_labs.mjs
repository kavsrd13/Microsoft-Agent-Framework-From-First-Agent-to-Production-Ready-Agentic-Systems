...
function materialId(file) {
  return `material-${file.replace(/[^a-zA-Z0-9]+/g, "-").replace(/^-|-$/g, "")}`;
}

function renderSupportingMaterials(lab) {
  if (!(lab.support || []).length) return "";

  const materials = lab.support.map((file) => {
    const absolutePath = path.resolve(ROOT, file);
    const relativePath = path.relative(ROOT, absolutePath);
    if (relativePath.startsWith("..") || path.isAbsolute(relativePath)) {
      throw new Error(`Supporting file is outside the repository: ${file}`);
    }
    if (!fs.existsSync(absolutePath)) {
      throw new Error(`Supporting file is missing: ${file}`);
    }

    const source = fs.readFileSync(absolutePath, "utf8");
    const id = materialId(file);
    return `<details class="embedded-material"><summary><code>${escapeHtml(file)}</code></summary><p>This material is included in this page. Review it here, then download a copy only if you need a local file.</p><p><button type="button" class="download-material" data-material-id="${id}" data-file-name="${escapeHtml(path.basename(file))}">Download a copy</button></p><textarea id="${id}" class="material-source" hidden>${escapeHtml(source)}</textarea>${renderCode(source, languageFor(file))}</details>`;
  }).join("");

  return `<h3>Supplied supporting files</h3><p>All required text materials are embedded below, so the lab works from a local HTML file and from GitHub Pages without relying on a separate download link.</p>${materials}`;
}
...
function renderFiles(lab, sources) {
  ...
  const support = renderSupportingMaterials(lab);
...
.optional-lab{border-top:4px solid var(--warning);background:#fffdf5}.optional-badge{align-self:flex-start;margin-top:.7rem;padding:.18rem .55rem;color:#7a5200;background:#fff1b8;border:1px solid #e7c65b;border-radius:999px;font-size:.78rem;font-weight:700}.embedded-material{margin:1rem 0;padding:0 1rem 1rem;border:1px solid var(--line);border-radius:10px;background:#fff}.embedded-material summary{padding:1rem 0;cursor:pointer;font-weight:700;color:#3730a3}.embedded-material .code-shell{margin-bottom:0}.download-material{padding:.45rem .8rem;color:#fff;background:var(--accent);border:0;border-radius:7px;cursor:pointer}.download-material:hover{background:#3730a3}
...
function validateGenerated() {
  ...
  for (const lab of labs) {
    ...
    for (const material of lab.support || []) {
      if (!content.includes(`data-material-id="${materialId(material)}"`)) throw new Error(`${slugFile(lab)} does not embed ${material}`);
      if (content.includes(`href="../${material}"`)) throw new Error(`${slugFile(lab)} still links to ${material} instead of embedding it`);
    }
  }
