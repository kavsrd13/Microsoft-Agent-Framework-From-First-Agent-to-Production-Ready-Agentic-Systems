...
<main class="exercise">
  <header class="lab-header"><a class="back-link" href="index.html">← Course lab index</a><h1>Lab ${displayId(lab)}: ${escapeHtml(lab.title)}</h1><p class="lab-header-desc">${escapeHtml(lab.why)}</p><div class="lab-header-meta"><span class="lab-meta-pill">Level ${lab.level}</span><span class="lab-meta-pill">${lab.duration}</span><span class="lab-meta-pill">${escapeHtml(lab.phase)}</span><span class="lab-meta-pill">${escapeHtml(lab.resource)}</span></div></header>
  ...
  <nav class="lab-pager">${previous ? `<a href="${slugFile(previous)}">← Lab ${displayId(previous)}: ${escapeHtml(previous.title)}</a>` : "<span></span>"}${next ? `<a href="${slugFile(next)}">Lab ${displayId(next)}: ${escapeHtml(next.title)} →</a>` : `<a href="index.html">Return to index →</a>`}</nav>
...
function indexPage() {
  ...
  const sections = phases.map((phase) => {
    ...
    return `<section class="phase"><h2>${escapeHtml(phase)}</h2><div class="lab-grid">${phaseLabs.map((lab) => `<article class="lab-card"><div class="card-top"><span class="lab-number">${displayId(lab)}</span><label class="index-check"><input type="checkbox" data-lab-complete="${lab.id}"> Complete</label></div><h3><a href="${slugFile(lab)}">${escapeHtml(lab.title)}</a></h3><p>${escapeHtml(lab.why)}</p><div class="card-meta"><span>Level ${lab.level}</span><span>${lab.duration}</span><span>${escapeHtml(lab.resource)}</span></div><a class="start-button" href="${slugFile(lab)}">Open lab →</a></article>`).join("")}</div></section>`;
  }).join("");
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Microsoft Agent Framework course labs</title><link rel="stylesheet" href="assets/lab.css"></head><body><main class="index-shell"><header class="course-header"><p class="eyebrow">Hands-on Python course</p><h1>Microsoft Agent Framework: From First Agent to Production-Ready Agentic Systems</h1><p>${labs.length} documentation-aligned labs covering clients, tools, MCP servers, memory, observability, evaluation, RAG, workflows, multi-agent orchestration, hosting, identity, and optimization.</p><div class="progress-panel"><div><strong id="progress-count">0 of ${labs.length} labs complete</strong><span>Progress is stored only in this browser.</span></div><div class="progress-track"><div id="progress-bar"></div></div></div></header><section class="index-readiness"><h2>Before Lab 00</h2><ol><li>Clone or extract the complete repository.</li><li>Install Python 3.12, Azure CLI, Node.js 22, and VS Code.</li><li>Obtain a Foundry project endpoint, deployed model name, and required Azure roles from the instructor.</li><li>Open each lab in the recommended order below. Every page repeats the exact environment and library setup so it can also be used independently.</li></ol><p><a href="../README.md">Project README</a> · <a href="../LAB_GUIDE.md">Instructor lab guide</a> · <a href="../.env.example">Environment template</a> · <a href="../requirements.txt">Pinned requirements</a></p></section>${sections}<footer>Course files are independently authored training material. Each lab links to the relevant Microsoft Learn documentation and official samples.</footer></main><script src="assets/lab.js"></script></body></html>`;
}
