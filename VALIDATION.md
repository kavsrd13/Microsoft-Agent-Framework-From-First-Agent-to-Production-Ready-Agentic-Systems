...
Live validation used `AzureCliCredential()` after `az login`, the configured Foundry project endpoint, and the `gpt-5` deployment.
...
The local `.env` contains endpoints, deployment names, and the Gemini key. Authentication remains token-based through Azure identity; no Azure API key was added for Foundry.
